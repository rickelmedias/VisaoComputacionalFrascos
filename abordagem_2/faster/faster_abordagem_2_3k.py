...
DATA_YAML = str(PROJECT_ROOT / "data_3k.yaml")
OUTPUT_DIR = PROJECT_ROOT / "abordagem_2" / "faster" / "weights_3k"
...
# --- MAIN ---
def main():
    print(f"=== Faster R-CNN - Abordagem 2 - Dataset 3k ===")
    print(f"Config: {NUM_EPOCHS} Epochs | Patience {PATIENCE} | Batch {BATCH_SIZE}")
    ...
    train_loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, collate_fn=collate_fn)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        t_loss, t_box, t_cls = train_one_epoch(model, optimizer, train_loader, DEVICE, epoch)
